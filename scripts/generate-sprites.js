...
// Characters to build. Each lives in static/<dir>/ with its raw MUGEN .sff and
// .air files; frames are written to static/<dir>/frames/. Adding a new MUGEN
// character is just another entry here.
const CHARACTERS = [
	{ dir: 'kikyo', sff: 'kikyo.sff', air: 'kikyo.air', def: 'Kikyo.def' },
	{ dir: 'kuwabara', sff: 'Kuwabara.sff', air: 'Kuwabara.air', def: 'Kuwabara.def' },
	{ dir: 'luffy', sff: 'Luffy.sff', air: 'Luffy.air', def: 'Luffy.def' },
	{ dir: 'moon', sff: 'moon.sff', air: 'moon.air', def: 'sailor_moon.def' },
	{ dir: 'ranma', sff: 'Ranma.sff', air: 'Ranma.air', def: 'Ranma.def' },
	{ dir: 'goku', sff: 'sgoku.sff', air: 'sgoku.air', def: 'sgoku.def' }
];
...
/**
 * Decode one character's animations into static/<dir>/frames/ + manifest.json.
...
export function buildCharacter(character) {
	const charDir = join(STATIC_DIR, character.dir);
	const outDir = join(charDir, 'frames');
	const airText = readFileSync(join(charDir, character.air), 'utf-8');
	const defText = readFileSync(join(charDir, character.def), 'utf-8');
	...
	// needs this.
	const sffBuffer = readFileSync(join(charDir, character.sff));
	const actPath = sffBuffer.readUInt8(15) === 1 ? findDefaultPalette(charDir, defText) : null;
...
// Run the full boilerplate build only when invoked directly (`pnpm gen:sprites`).
// When imported (scripts/import-mugen.js) we just expose the helpers above.
