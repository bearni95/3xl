 ...
 *     → auto-bind a @3xl/data public/characters/<id>.json definition from the manifest
 ...
 *     static/characters/<id>.json survive — so this mode is safe to run
 *     repeatedly.
 *   wipe — delete ALL imported character data first (static/<id>/ folders and
 *     static/characters/*.json definitions, hand-tuned edits included), then
...
function importArchive(archivePath, taken) {
	...
	try {
		...
		// Extract the raw MUGEN moveset into a fetchable JSON in @3xl/data, served
		// at /data/<id>/mugen-moves.json. Always rewritten — it is generated data,
		// never hand-edited.
		if (cmdFile) {
			const moveset = extractMoveset(destDir, cmdFile, stFiles, manifest);
			const movesPath = join(DATA_DIR, id, 'mugen-moves.json');
			mkdirSync(dirname(movesPath), { recursive: true });
			writeFileSync(movesPath, JSON.stringify(moveset, null, '\t') + '\n', 'utf-8');
			console.log(`    ↳ ${moveset.moves.length} MUGEN move(s) → data/${id}/mugen-moves.json`);
		} else {
		...
		if (existsSync(defJsonPath)) {
			console.log(`    ↳ definition data/characters/${id}.json exists — kept as-is`);
		} else {
 ...
 * (@3xl/assets public/<id>/, but never the shared `auras`), movesets
 * (@3xl/data public/<id>/, but never the `characters` definitions dir) and the
 * definition JSON files themselves.
 */
function wipeImportedData() {
	...
	removeDirsIn(DATA_DIR, ['characters']);
	if (existsSync(DEFINITIONS_DIR)) {
		for (const f of readdirSync(DEFINITIONS_DIR)) {
			if (f.endsWith('.json')) rmSync(join(DEFINITIONS_DIR, f));
		}
	}
