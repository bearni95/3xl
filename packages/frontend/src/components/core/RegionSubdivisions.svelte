...
<!-- White ink, as on the bar: a crumb letters what it flies in white at 70% and is drawn to
	...
	The column is in two parts, and only the second of them moves: the place the map is open on
	with everything it has to say, then the level under it. The head is what the reader came to
	this column for — where they are, who holds it, what it is made of, and the two controls that
	act on the list — and a comarca of forty towns used to push every one of those off the top of
	the screen. So the head is pinned and the list scrolls under it, which is why this is a flex
	column with a height to divide rather than a run of rows as tall as its contents (see the
	`min-h-0` the caller hands it). -->
<div class={classNames('flex min-h-0 flex-col text-white', classes)}>
	<!-- The pinned part: everything down to and including the row of shares and the field the
		looking glass on it opens. `min-h-0` is not wanted here — this block is as tall as it is,
		and it is the list below that gives way. -->
	<div class="flex flex-none flex-col gap-0.5 px-2 pt-2">
		{#if current}
	...
	<!-- The level, and the whole of what moves in this column: it takes whatever height the
		heads above it have left and scrolls inside that, so a comarca of forty towns is read
		against a head that stays put rather than by pushing it off the screen. `min-h-0` is what
		lets it be shorter than its own contents — a flex item is floored at its content height
		without it, which is a column that scrolls the whole page instead of this box. The
		padding is the pinned block's, carried on so the rows line up with the shares above them,
		and the 0.5 at the top is the gap every other row of the column is spaced by. -->
	<div class="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto px-2 pb-2 pt-0.5">
		{#if searching}
