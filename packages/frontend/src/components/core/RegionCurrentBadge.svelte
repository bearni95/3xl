...
<div class={classNames('flex flex-col justify-center text-white', classes)}>
	{#if named}
		<!-- The name and nothing at its far end, which is what `named` is (see above): the whole
			width goes to the place and the song, and the box the town has waiting is read on the
			tab under this that is about nothing else. No `boxWidth` for the same reason — there is
			no box here to be sized. -->
		<RegionListRow
			row={named}
			current
			onSelect={press}
		...
		>
			<svelte:fragment slot="line">
				<MusicLine showName={named.showName} />
			</svelte:fragment>
		</RegionListRow>
	{/if}
</div>
