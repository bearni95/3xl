<script lang="ts">
	...
	onMount(async () => {
		const [festesResult, showsResult] = await Promise.allSettled([
			fetch('/data/festes-locals.json').then(
				(response) => response.json() as Promise<FestesCollection>
			),
	...
	// Today's date key, built against the calendar's year so it matches the baked
	// dataset even if the wall-clock year has moved on. Month is 0-based for isoDate.
	$: today = collection ? isoDate(collection.year, new Date().getMonth(), new Date().getDate()) : '';
	...
	$: todayFestes = ((today && index.get(today)) || []).map(
		(festa: MunicipalityFesta) => ({ festa, show: showByMunicipality.get(festa.id) })
	);
