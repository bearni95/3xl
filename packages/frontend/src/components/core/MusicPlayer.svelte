<script lang="ts">
	...
	// The map's music: the show themes vendored in @3xl/assets, one plate in the
	// corner with the three things a two-song player needs — what is loaded, a
	// play/pause, and the swap to the other one.
	//
	// It reads as the town panel below it does — the same black plate, the same
	// glyph tile at the left end with two lines of text beside it — because the two
	// stand in one corner and are one stack of plates, not two kinds of thing. The
	// plate is black for the town panel's reason: it sits over satellite imagery,
	// and the lettering has to be read off the plate rather than off the terrain.
	//
	// No state of its own: which track is loaded and whether it is running belong to
	// musicService, which owns the audio element so that the sound outlives anything
	// that happens on screen.

	export let classes: string = '';

	// The show's name for the second line, from the same baked shows.json the statues
	// read. Already loaded by everything else on this page, so this is a no-op there;
	// asked for anyway, so the player is drawable wherever it is put.
	onMount(() => {
		void loadShowLogos();
	});
	...
	$: state = $music;
	$: showIcon = showIconName(state.track.showId);
	$: showName = state.track.showId ? ($showLogos.get(state.track.showId)?.name ?? null) : null;
</script>

<div
	class={classNames('flex items-center gap-2 rounded-lg bg-black p-2 text-white shadow-xl', classes)}
>
