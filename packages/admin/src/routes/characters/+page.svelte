<script lang="ts">
	...
	async function load(character: CharacterOption) {
		...
		try {
			const [defRes, manifestRes, movesRes] = await Promise.all([
				...
				fetch(`/data/${character.id}/mugen-moves.json`)
			]);
	...
	async function handleSave(event: CustomEvent<CharacterDefinition>) {
		...
		saveError = '';
		try {
			const res = await fetch(`/api/characters/${event.detail.id}`, {
				method: 'POST',
			...
			<section class="card bg-base-100 shadow-xl">
				<div class="card-body gap-4">
					<div class="flex items-center gap-3">
						...
						<span class="badge badge-ghost font-mono text-xs">/data/{selected.id}/mugen-moves.json</span>
					</div>
					<p class="text-sm opacity-70">
