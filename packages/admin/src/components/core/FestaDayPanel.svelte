<script lang="ts">
	...
	// Whether the date falls in the sync window (today → year end) — for a hint.
...
>
	...
	{#if date}
		<div class="flex items-center justify-between gap-2 border-b border-base-300 px-4 py-2 text-xs">
			<span class="opacity-70">
				{festes.length}
				{festes.length === 1 ? 'municipi' : 'municipis'}
			</span>
			{#if inWindow}
				<span class="badge badge-success badge-sm">Es sincronitza</span>
			{:else}
				<span class="badge badge-ghost badge-sm">Fora de finestra</span>
			{/if}
		</div>
