<script lang="ts">
	...
	// Emits the computed per-id status and the per-id Supabase rarity whenever
	// they change, so the parent can pass both down to each grid card (the card
	// renders the sync badge and the rarity editor).
	const dispatch = createEventDispatcher<{
		statuschange: Map<string, CharacterTemplateStatus>;
		raritychange: Map<string, number>;
	}>();
	...
	// Publish each character's Supabase rarity (default for rows never set), so the
	// grid cards can seed their editors. Empty until the remote list has loaded.
	$: dispatch(
		'raritychange',
		loaded ? new Map(remote.map((t) => [t.id, t.rarity ?? DEFAULT_RARITY])) : new Map()
	);
...
<section class="card bg-base-100 shadow-xl">
	<div class="card-body gap-3">
		...
		<p class="text-sm opacity-70">
			...
			remote rows that no longer exist locally. Each card below is badged with its sync state, and
			carries its own <strong>rarity</strong> editor — rarity lives only in Supabase, and the sync
			leaves it untouched.
		</p>
